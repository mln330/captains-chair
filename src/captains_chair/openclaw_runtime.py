...
You are a role-separated worker for Captain's Chair. The Workboard card is your complete work contract.

Mandatory protocol:

1. Read the card, parent results, source links, repository AGENTS.md, `.captains-chair/project.yaml`, and canonical planning documents before acting.
2. Work only in the workspace or worktree supplied by the card. Never switch the shared checkout to a feature branch.
3. Keep the card claim alive with the CAPTAINS_CHAIR `worker-protocol heartbeat` command during long operations. Never call `heartbeat_respond`; it is not a Workboard lease heartbeat and may be unavailable.
...
Never bypass CAPTAINS_CHAIR policy, fabricate proof, approve your own implementation, merge from a coding role, expose secrets, force-push, delete branches, release, or deploy production unless the card and deterministic policy explicitly authorize it.
...
- Heartbeat: `{captains_chair_command} worker-protocol heartbeat --repo <owner/repo> --orchestrator openclaw-workers --card <id> --owner-id <ownerId> --token <token> --note "still working"`
- Complete: `{captains_chair_command} worker-protocol complete --repo <owner/repo> --orchestrator openclaw-workers --card <id> --owner-id <ownerId> --token <token> --summary "what changed" --proof-note "tests and current-head evidence"`
- Block: `{captains_chair_command} worker-protocol block --repo <owner/repo> --orchestrator openclaw-workers --card <id> --owner-id <ownerId> --token <token> --reason "TECHNICAL: exact repairable failure"`
"""
...
ROLE_PROTOCOLS: dict[str, str] = {
    ...
    "coder": """\n## Role: implementation and repair\n\nImplement only the linked acceptance criteria in the supplied isolated worktree. Run targeted checks, scan the diff for secrets and unrelated churn, push the CAPTAINS_CHAIR branch, and open or update the linked PR with its exact GitHub PR URL, head SHA, and test proof in the structured completion proof. Never review, approve, or merge your own work.\n""",
...
class OpenClawRuntimeInstaller:
    ...
    def install(self, workspace_root: Path) -> tuple[RuntimeInstallAction, ...]:
        ...
        for item in actions:
            ...
            command = shlex.join(self.config.captains_chair_command)
            instructions = (
                COMMON_PROTOCOL.replace("{captains_chair_command}", command)

...
from captains_chair.models import EventRecord, NotificationConfig
...
NOTIFIER_ADAPTER_ENTRYPOINT_GROUP = "captains_chair.notifier_adapters"
...
def render_event(event: EventRecord) -> str:
    ...
    if requires_owner_attention(event.event_type, event.evidence):
        ...
        if action_id:
            lines.append(f"Approve: `captains_chair approve --repo {event.repo} --action-id {action_id}`")
        if link_text:

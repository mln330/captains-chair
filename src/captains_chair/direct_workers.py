...
from captains_chair.harness import strict_output_schema
from captains_chair.json_tools import decode_first_json
from captains_chair.models import StrictModel
from captains_chair.orchestration import QueueCard
...
class CommandWorkerExecutor:
    ...
    def _run_codex(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="captains-chair-worker-") as temp_dir:
    ...
    def _run_openclaw(
        ...
        *,
        ...
        command = [
            ...
            "--session-key",
            f"agent:{card.agent_id}:captains-chair:worker:{card.id}:{attempt_id}",
            "--message",
            prompt,
            "--json",
            "--timeout",

...
from make_it_so.models import RepoConfig
from make_it_so.orchestration import QueueCard, QueueCardSpec, QueueStatus

CANARY_PROOF_PREFIX = "MAKE_IT_SO_CANARY_PROOF:"
...
def canary_board_id(repo: RepoConfig) -> str:
    ...
    return f"make_it_so-canary-{repo.full_name.replace('/', '-').lower()}"
...
def build_canary_spec(
    ...
    *,
    ...
    return QueueCardSpec(
        key=f"make_it_so:canary:{repo.full_name.lower()}:{normalized_id}",
        title=f"MAKE_IT_SO runtime canary: {repo.full_name}",
        notes=(
        ...
        priority="low",
        labels=("make_it_so", "runtime-canary", f"repo:{repo.full_name.lower()}", f"canary:{normalized_id}"),
        agent_id=worker_id,

...
PR bodies and planning docs may describe defects in a legacy Captain. That text is not evidence that the current MAKE_IT_SO runtime is still defective. Choose control-plane system maintenance only when current run evidence demonstrates a present MAKE_IT_SO failure. If the current runtime has durable-document guards and the remaining defect is stale repo documentation, choose a managed-repository plan update in a fresh branch.

...
from make_it_so.adapters import CallbackUsageTelemetryAdapter, UsageTelemetryAdapter
...
from make_it_so.config import load_config, write_json_schema
from make_it_so.courses import CourseStore, planning_session, readiness_report
from make_it_so.engine import ControlPlaneEngine, ModelCallSuppressedError
from make_it_so.github import GhGitHubProvider
from make_it_so.harness import build_harness
...
from make_it_so.notifications import NotificationError, Notifier, build_notifier, render_event
from make_it_so.openclaw_runtime import OpenClawRuntimeInstaller
from make_it_so.openclaw_usage import DEFAULT_SESSION_LIMIT, sync_openclaw_sessions
...
from make_it_so.state import LeaseBusyError, StateStore
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="make-it-so", description="Make It So control plane")
    parser.add_argument("--config", type=Path, default=Path("~/.config/make-it-so/config.yaml").expanduser())
...
def _default_direct_orchestrator_config(config: AppConfig, repo: RepoConfig) -> DirectOrchestratorConfig:
    ...
    return DirectOrchestratorConfig(
        ...
        board_prefix="make-it-so-direct",
        workers=workers,
    )
...
def _preflight_worker_protocol(adapter: Any) -> dict[str, Any] | None:
    ...
    command_value = getattr(runtime_config, "make_it_so_command", None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "recover-pr":
            ...
            if not branch.startswith("make_it_so/"):
                raise ValueError("recovery PR is not on an MAKE_IT_SO-owned branch")
            ...
            event = state.record_event(
                ...
                reason="The exact approved action opened a valid PR before cleanup failed; MAKE_IT_SO recovered it without creating a duplicate.",
        ...
        if args.command == "schedule":
            ...
            spec = ScheduleSpec(
                name=("make-it-so-watch-" if args.watch else "make-it-so-") + repo.full_name.replace("/", "-").lower(),
                argv=(
                    "make-it-so",
                    "--config",
        ...
        if args.command == "orchestration-schedule":
            ...
            _board_id(config, repo.full_name)
            spec = ScheduleSpec(
                name="make-it-so-dispatch-" + repo.full_name.replace("/", "-").lower(),
                argv=(
                    "make-it-so",
                    "--config",
                    str(args.config.resolve()),
        ...
        print(f"make_it_so failed: {exc}", file=sys.stderr)
        return 3

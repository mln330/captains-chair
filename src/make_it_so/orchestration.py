...
from make_it_so.qa import QASelection, select_qa
...
class ClaimingWorkerLifecycleAdapter(WorkerLifecycleAdapter, Protocol):
    """Full portable lifecycle for runtimes where Make It So owns claims."""
...
class WorkflowOrchestrator:
        ...
        *,
        ...
        try:
            self.adapter.ensure_board(
                ...
                f"Make It So work for {repo.full_name}",
                repo.local_path,
            )
            ...
            self.adapter.complete_card(
                root.id,
                summary="MAKE_IT_SO materialized the policy-gated worker workflow.",
                proof=(
                    {
                        "status": "passed",
        ...
        for card in cards:
            ...
            if stage in {
                ...
                repair = self.adapter.create_card(
                    board_id,
                    QueueCardSpec(
                        key=f"make_it_so:repair:{card.id}:{_failure_count(card)}",
        ...
        for workflow, workflow_cards in groups.items():
            ...
            for profile in context.selection.profiles:
                ...
                card = self.adapter.create_card(
                    board_id,
                    QueueCardSpec(
                        key=f"make_it_so:{workflow}:qa:{_qa_key(profile.key)}:{context.head_sha[:12]}",
    ...
    def _create_fresh_retry(
        ...
        retry = self.adapter.create_card(
            ...
            QueueCardSpec(
                key=f"make_it_so:retry:{card.id}:{attempt}",
                ...
                notes=(
                    ...
                    "Complete only through the MAKE_IT_SO worker lifecycle helper with current-head evidence."
                ),
                status=QueueStatus.READY,
                priority="high",
...
def _root_notes(repo: RepoConfig, decision: PlanDecision, action_id: str) -> str:
    ...
    return (
        f"Repository: {repo.full_name}\n"
        f"MAKE_IT_SO workflow: {action_id}\n"
        f"Action: {decision.action.value}\n"
    ...
    *,
    ...
    contracts = {
        ...
        WorkStage.IMPLEMENTATION: (
            ...
            "push the MAKE_IT_SO branch, open or update a PR, and include its exact GitHub PR URL and head SHA in completion proof. "
        ...
        WorkStage.MERGE: (
            "Re-read the live PR and run the deterministic MAKE_IT_SO merge gate. Merge only when AUTO_MERGE_ALLOWED is anchored to the "

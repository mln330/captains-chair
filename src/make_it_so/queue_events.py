...
from make_it_so.models import EventRecord, RepoConfig
from make_it_so.orchestration import QueueCard, QueueStatus, WorkStage, classify_blocker
...
def project_queue_events(
    ...
    *,
    ...
    for card_id in protocol_retries:
        ...
        events.append(
            state.record_event(
                ...
                reason=(
                    "The worker runtime ended the run without the mandatory MAKE_IT_SO completion proof; "
    ...
    for transition in transitions:
        ...
        if card.status == QueueStatus.RUNNING and stage == WorkStage.IMPLEMENTATION.value:
            ...
            next_action = {
                ...
                WorkStage.POST_MERGE.value: "This workflow is complete; the Number One can select the next work item.",
            }[stage]
    ...
    # Re-emit only at ladder levels so the Number One escalates without spamming every
    ...
    return state.record_event(
        ...
        evidence={
            ...
            "next_action": (
                ...
                f"`make_it_so orchestrate unblock --repo {repo.full_name} --card {card.id}` and run "
                "`make_it_so orchestrate reconcile`."
            ),
            "links": [card.source_url] if card.source_url else [],
            "blocker": blocker,
            "owner_required": True,
    ...
    *,
    ...
    actions = (
        (
            ...
            "The Number One reclaimed the failed card and will retry it without owner intervention.",
        ...
        (
            ...
            "Number One recovery was queued automatically.",
            "The technical retry budget was exhausted, so the Number One will replan the failure.",
            "The Number One recovery worker will inspect the evidence and select the next repairable action.",
        ),
    )
...
def _diagnostic_action(row: dict[str, Any], kind: str) -> str:
    ...
    if kind == "missing_proof":
        return "Reconcile the queue; MAKE_IT_SO will retry the incomplete worker handoff or route recovery."
...
def _has_protocol_recovery_comment(card: QueueCard) -> bool:
    return "mandatory MAKE_IT_SO completion proof" in _latest_comment(card)

...
class WorkerOrchestrationConfig(StrictModel):
    ...
    board_prefix: str = Field(default="make-it-so", min_length=1)
...
class OpenClawWorkboardConfig(WorkerOrchestrationConfig):
    ...
    def command_must_not_be_empty(self) -> OpenClawWorkboardConfig:
        if not self.make_it_so_command or any(not item.strip() for item in self.make_it_so_command):
...
class RepoConfig(StrictModel):
    ...
    planning_doc: str
    project_manifest: str = ".make-it-so/project.yaml"

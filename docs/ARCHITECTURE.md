...
- unblock, reclaim, and Number One recovery routing
- diagnostics
...
The direct Number One final-review contract also has an `owner_blocker` field. It must use
...
the Number One reports a stalled technical state and waits for changed evidence or an
...
health failures are reported as `Number One HANDLING` with the automatic next action. A
...
and Number One-recovery routing as idempotent `Number One HANDLING` events, so autonomous progress is
...
lease: worker heartbeats and completions can proceed while the Number One reconciler is
running.

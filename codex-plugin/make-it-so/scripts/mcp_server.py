"""Minimal stdio MCP bridge for the Make It So Codex plugin.

The bridge exposes the installed Make It So CLI rather than duplicating
...
from make_it_so.models import Course
...
TOOLS: tuple[dict[str, Any], ...] = (
    {
        "name": "make_it_so_doctor",
        "description": "Validate Make It So configuration and runtime prerequisites.",
        "inputSchema": _object_schema((), {}),
    },
    {
        "name": "make_it_so_baseline",
    ...
    {
        "name": "make_it_so_cycle",
        "description": "Run one bounded Make It So cycle; policy gates determine whether it can mutate.",
        "inputSchema": _object_schema(
            ("repo", "harness"),
            {
                "repo": {"type": "string"},
    ...
    {
        "name": "make_it_so_course_approve",
    ...
    {
        "name": "make_it_so_worker_heartbeat",
    ...
    {
        "name": "make_it_so_worker_complete",
...
def _config_path(arguments: dict[str, Any]) -> str:
    value = str(arguments.get("config_path") or os.environ.get("MAKE_IT_SO_CONFIG") or "").strip()
    if not value:
        raise ValueError("config_path is required or MAKE_IT_SO_CONFIG must be set")
    return value
...
def _run_tool(name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    command: list[str]
    sidecar_methods = {
        "make_it_so_course_create": "course.create",
        "make_it_so_course_readiness": "course.get",
        "make_it_so_course_answer": "course.requirement",
        "make_it_so_course_approve": "course.approve",
        "make_it_so_course_pause": "course.pause",
        "make_it_so_course_resume": "course.resume",
        "make_it_so_course_checkpoint": "course.checkpoint",
        "make_it_so_ready_work": "course.ready_work",
    }
    ...
    if name == "make_it_so_doctor":
        command = ["doctor"]
    elif name == "make_it_so_baseline":
    ...
    elif name == "make_it_so_status":
        if not repo:
            raise ValueError("status requires repo")
        command = ["status", "--repo", repo]
    elif name == "make_it_so_planning_session":
    ...
    elif name == "make_it_so_usage":
        command = ["usage", "report"]
        if repo:
            command.extend(["--repo", repo])
    elif name == "make_it_so_attention_ack":
    ...
    elif name == "make_it_so_worker_discover":
        if not repo:
            raise ValueError("worker discovery requires repo")
        command = ["orchestrate", "status", "--repo", repo]
    elif name.startswith("make_it_so_worker_"):
        action = name.removeprefix("make_it_so_worker_")
        if (
            not repo
    ...
    completed = subprocess.run(
        [sys.executable, "-m", "make_it_so", "--config", config, *command],
        check=False,
        capture_output=True,
        text=True,
        timeout=1800,
        ...
        _send(
            {
                ...
                "result": {
                    ...
                    "serverInfo": {"name": "make-it-so", "version": "0.2.0"},
                },
            }
        )
        return
